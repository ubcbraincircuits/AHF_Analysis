...
varOpen.working_folder = 'D:\20180824\processed\';

varOpen.file(1).rep = varOpen.working_folder; % load the text files
varOpen.file(2).rep = 'D:\20180924'; 
varOpen.file(3).rep = 'D:\20180924';
...
varOpen.filesave = 'D:\20180824\processed\20180824_29'; % load the videos 
...
varOpen.varProc.run_list=[1:22, 24:37, 40:120, 122:136]; % recommend excluding 39th expe, shift in calcium frames 
...
varOpen.Video.positionWin = []; % position (in seconds)
...
varOpen.Video.description = 0 % show (1) or don't show (0) descriptions of the trials at the top of the plot (like the period, wait time, etc) 
...
varOpen.ave.jonction = 1 % concerning the overlap in trials (deletes the trials that overlap) 
...
varOpen.ave.seed_pixel = []; % Pixel dim. of seed you would like to analyze ([x,y]).  Leave it as [] for the interactive plot.  
...
for m= 1:length(qtype_list)
    ...
    [varOpen,expe] = FARM_averaging_teri_new_nomovement(varOpen,expe);
    catch
...
varOpen.singletrial.trial_interval = [] % state the interval of trials you want to view [] indicates all trials ([min:max]) 
